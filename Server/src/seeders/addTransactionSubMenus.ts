/**
 * One-shot script: inserts "Vendor Payments" and "Customer Payments" sub-menus
 * under the Transactions parent menu, and grants full permissions to
 * SUPERADMIN (global) + ADMIN (per-store).
 * Safe to re-run — uses findOrCreate / upsert throughout.
 *
 * Run with: npx ts-node src/seeders/addTransactionSubMenus.ts
 */
...
async function run() {
  ...
  const transactionsMenu = await Menu.findOne({ where: { link: "/transactions" } });
  if (!transactionsMenu) {
    console.error('Parent "Transactions" menu not found. Run addTransactionsMenu.ts first.');
    process.exit(1);
  }
  ...
  const subMenuDefs = [
    { name: "Vendor Payments",   link: "/transactions/vendor",   sort_order: 5.871 },
    { name: "Customer Payments", link: "/transactions/customer", sort_order: 5.872 },
  ];

  for (const def of subMenuDefs) {
    const [menu, created] = await Menu.findOrCreate({
      where: { name: def.name, parent_id: transactionsMenu.id },
      defaults: {
        name:            def.name,
        link:            def.link,
        parent_id:       transactionsMenu.id,
